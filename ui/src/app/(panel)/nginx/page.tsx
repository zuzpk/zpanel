"use client"
import { Box, Button, Cover, Select, Table, Text, useDrawer, useToast, Variant } from '@zuzjs/ui';
...
import { VirtualHost, PubEvent } from '@/types';
...
import VirtualhostEditor from './virtualhost-editor';
...
const Page : React.FC = (_props) => {
    ...
    const editBlock = async (r: VirtualHost) => {

        drawerId.current = drawer.right(<VirtualhostEditor 
            source={r} 
            onSave={loadData}
            />)
        ...
        //     drawer.right(<Box as={`flex cols w:90vw`}>
        //         <CodeEditor 
        //             files={[
        //                 {
        //                     token: r.id,
    ...
    return <><Box as={`w:calc[100vw - 60px] h:100dvh no-overflow flex cols p:$page-padding gap:15`}>
        ...
        <Box as={`flex aic rel`}>
            ...
            <Box as={`flex aic gap:5`}>
                <Button 
                    onClick={() => drawerId.current = drawer.right(<VirtualhostEditor />)}
                    as={`--btn`} 
                    icon={`add`}>New Server</Button>
            </Box>
        </Box>
        <Table 
            animateRows={true}
            ...
            schema={[
                {
                    ...
                    render: r => <Box as={`flex cols gap:2`}>
                        ...
                        <Text as={`s:14 opacity:0.5`}>{r.rootPath}</Text>
                    </Box>,
                },
                {
                    id: `action`,

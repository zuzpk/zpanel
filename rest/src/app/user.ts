import { APP_NAME, BASH_ROOT, SESS_NAME } from "@/config"
import { Encode, execSyncSudo, fromHash, headers, log, sendPush, toHash, withoutSeperator, withSeperator } from "@/lib"
import { LinuxGroup, LinuxUser, User, UserStatus, UserType } from "@/lib/types"
import zorm, { PushTokens, Users } from "@/zorm"
...
export const createSystemUser = (username: string): boolean => {

    log.info(APP_NAME, `Creating System User...`)

    try {
        // Check if user already exists to avoid errors
        execSync(`sudo id -u ${username}`, { stdio: 'ignore' });
        log.error(APP_NAME, `User ${username} already exists.`);
        return true;
    } catch (err) {
        // -m: create home dir
        // -s: set shell to nologin for security
        // -r: create a system account
        execSyncSudo(`useradd -m -r -s /usr/sbin/nologin ${username}`);
        log.info(APP_NAME, `System user ${username} created successfully.`);
        return true;
    }
};
...
export const getLinuxUsers = (): LinuxUser[] => {
  try {
    ...
    try {
      ...
      log.error(APP_NAME, 'Cannot read /etc/shadow (run as root for hasPassword info)');
    }
    ...
    const users: LinuxUser[] = lines
      ...
      .filter((user: LinuxUser) => user.username && user.username.trim() !== ''); // skip empty lines

    return users;
  } catch (err) {
    log.error(APP_NAME, 'Failed to list users:', err);
    log.error(APP_NAME, `Cannot read user information: ${err instanceof Error ? err.message : String(err)}`);
    return []
  }
};
...
export const getLinuxGroups = (): LinuxGroup[] => {
  try {
    ...
    log.error(APP_NAME, 'Failed to read /etc/group:', err);
    log.error(APP_NAME, `Cannot list groups: ${err instanceof Error ? err.message : String(err)}`);
    return []
  }
};

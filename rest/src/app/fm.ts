...
import { dynamic, exists } from "@zuzjs/core"
import { Encode, execSyncSudo } from '@/lib/core';
...
export const CreateFolder = async (req: Request, resp: Response) => {
  ...
  if ( !exists(target) ){
    return resp.send({
      error: `dirExist`,
      target,
      message: `A dir with that name already exist`
    })
  }

  execSyncSudo(`mkdir -p ${target}`)
  
  return resp.send({
    kind: `dirCreated`,
    message: `Nice and easy :) Directory created successfully.`,
    target
  })
  
}

export const CreateEmptyFile = async (req: Request, resp: Response) => {
